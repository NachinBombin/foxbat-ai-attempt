...
-- ============================================================
-- TUNING  (foxbat-specific, not LVS flight params)
-- ============================================================

ENT.NFP_SkyHeightAdd     = 2500
ENT.NFP_AltMin           = 1800
ENT.NFP_AltMax           = 3800
ENT.NFP_WanderRadius     = 3000
ENT.NFP_WanderInterval   = { 6, 14 }
ENT.NFP_CruiseDiveChance = 0.35
ENT.NFP_SearchWindow     = 12

ENT.NFP_DiveSpeed        = 1800
ENT.NFP_DiveAccel        = 0.022
ENT.NFP_DiveTrackRate    = 0.08
ENT.NFP_WobbleAmpH       = 160
ENT.NFP_WobbleAmpV       = 110
ENT.NFP_WobbleSpeedH     = 4.2
ENT.NFP_WobbleSpeedV     = 3.0
...
function ENT:Initialize()
    -- Read spawner-supplied parameters
    ...
    self:SetAngles( Angle( 0, nfpStartYaw, 0 ) )

    -- Let LVS base_anim / lvs_base handle physics init
    -- (lvs_base Initialize sets up the physobj, collision, movetype etc.)
    ...
    -- Wander (used only to pick _nfpAITargetAng during cruise)
    self._nfpNextWander      = CurTime() + math.Rand( 3, 7 )
    self._nfpSearchExpiry    = 0

    -- Initial wander target: straight ahead at cruise altitude band midpoint
    local nfpCruiseAlt = nfpGround + (self.NFP_AltMin + self.NFP_AltMax) * 0.5
    self._nfpWanderTarget = nfpSpawnPos + self._nfpCallDir * self.NFP_WanderRadius
    self._nfpWanderTarget.z = nfpCruiseAlt
...
-- ============================================================
-- THINK — only manages mode transitions and dive.
-- Cruise flight is driven entirely by LVS PhysicsSimulate via
-- _nfpAITargetAng written below. No SetPos. No SetAngles.
-- ============================================================
...
-- ============================================================
-- CRUISE — picks wander waypoints, writes _nfpAITargetAng only.
-- LVS PhysicsSimulate chases that angle with real torque.
-- ============================================================
...
function ENT:NFP_ThinkCruise( nfpNow, nfpDT )
    ...
    if nfpNow >= self._nfpNextWander then
        ...
        local nfpDist   = math.Rand( self.NFP_WanderRadius * 0.3, self.NFP_WanderRadius )
        ...
        local nfpAlt    = self._nfpGroundZ + math.Rand( self.NFP_AltMin, self.NFP_AltMax )

        self._nfpWanderTarget = Vector( nfpWX, nfpWY, nfpAlt )
        self._nfpNextWander   = nfpNow + math.Rand( self.NFP_WanderInterval[1], self.NFP_WanderInterval[2] )

        -- Chance to arm dive search
    ...
    -- ── Write target angle for LVS CalcAero to chase ────────
    -- This is THE only movement control during cruise.
    -- CalcAero in nfp_base_fighterplane/init.lua reads _nfpAITargetAng
    -- and applies real pitch/yaw/roll torque to the physobj.
    local nfpToTarget = self._nfpWanderTarget - nfpPos
    if nfpToTarget:LengthSqr() > 1 then
        self._nfpAITargetAng = nfpToTarget:GetNormalized():Angle()
    end
end

-- ============================================================
-- DIVE_SEARCH — keep writing cruise target angle while scanning
-- ============================================================

function ENT:NFP_ThinkDiveSearch( nfpNow, nfpDT )
    -- Keep cruise target angle updating so the plane keeps flying
    self:NFP_ThinkCruise( nfpNow, nfpDT )

    if nfpNow >= self._nfpSearchExpiry then
        self._nfpMode = "cruise"
        self:NFP_Debug( "DIVE_SEARCH expired" )
        return
    end
...
-- ============================================================
-- DIVE_COMMIT — telegraph then switch to full dive.
-- During dive, _nfpAITargetAng is overridden with steep dive angle
-- so LVS physics pitches the nose down naturally.
-- After close enough, switch to direct position drive (NFP_UpdateDive).
-- ============================================================

function ENT:NFP_ThinkDiveCommit( nfpNow, nfpDT )
    if self._nfpDiveExploded then return end

    -- Telegraph: aim nose at target via _nfpAITargetAng — LVS will pitch down
    if nfpNow < self._nfpDiveCommitTime then
        if IsValid( self._nfpDiveTarget ) then
            local nfpDir = (self._nfpDiveTarget:GetPos() - self:GetPos()):GetNormalized()
            self._nfpAITargetAng = nfpDir:Angle()
        end
        return
    end
    ...
    if not self._nfpDiveActive then
        ...
        -- Disable LVS AI angle chase — dive uses direct velocity control
        self._nfpAITargetAng = nil
...
-- ============================================================
-- DIVE UPDATE — direct velocity drive, bypasses LVS physics.
-- _nfpAITargetAng is nil here so CalcAero does nothing to angle.
-- We drive the physobj velocity directly for precision.
-- ============================================================
...
function ENT:NFP_UpdateDive( nfpNow, nfpDT )
    ...
    -- Drive via physobj velocity — LVS physobj is already awake
    local nfpPhys = self:GetPhysicsObject()
    if IsValid( nfpPhys ) then
        local nfpDiveVel = nfpDir * self._nfpDiveSpeedNow + nfpWobble
        nfpPhys:SetVelocity( nfpDiveVel )
        -- Face travel direction by directly setting angle velocity toward target
        local nfpFaceAng = nfpDiveVel:GetNormalized():Angle()
        nfpFaceAng.r = 0
        local nfpCurAng = self:GetAngles()
        local nfpAngDiff = nfpFaceAng - nfpCurAng
        nfpAngDiff:Normalize()
        nfpPhys:SetAngleVelocity( Vector( nfpAngDiff.p, nfpAngDiff.y, nfpAngDiff.r ) * 10 )
    end

    -- Geometry hit
    local nfpTr = util.TraceLine({
        start  = nfpMyPos,
        endpos = nfpMyPos + nfpDir * self._nfpDiveSpeedNow * nfpDT * 2,
        filter = self,
        mask   = MASK_SOLID,
    })
    if nfpTr.Hit then
